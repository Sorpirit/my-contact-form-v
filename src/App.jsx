...
function App() {
  ...
  const onResponse = (responseMessage, successful) => {
    setSendingForm(false);
    setResponse(true);
    if (!successful) setFailed(true);
    ...
    <div>
      ...
      {response ? (
        <Fade in={response}>
          {!failed ? (
            <Alert
              className="mb-3 mx-5 bg-success d-flex justify-content-center"
              variant="sucsses"
              onClose={() => setResponse(false)}
              dismissible
            >
              <Alert.Heading>
                Email sent! Check your email for a prise.
              </Alert.Heading>
            </Alert>
          ) : (
            <Alert
              className="mb-3 mx-5 bg-danger d-flex justify-content-center text-white text-center"
              variant="danger"
              onClose={() => setResponse(false)}
              dismissible
            >
              <Alert.Heading>
                Oh snap! You got an error! Try again later.
              </Alert.Heading>
            </Alert>
          )}
        </Fade>
      ...
      <Form onSubmit={sendEmail} variant="dark">
        ...
        <Button
          ...
          type="submit"
          disabled={
            sendingForm || captchaKey == null || email == null || email === ""
          }
        >
      ...
      <ReCAPTCHA
        ...
        onChange={(value) => CaptchaCheck(value)}
      />
    </div>
  );
}
